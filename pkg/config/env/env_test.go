...
import (
	...
	"github.com/go-primus/primus/pkg/config"
	"github.com/go-primus/primus/pkg/config/file"
)
...
func TestEnvWithPrefix(t *testing.T) {
	...
	// set env
	prefix1, prefix2 := "PRIMUS_", "FOO"
	envs := map[string]string{
		prefix1 + "SERVICE_NAME": "primus_app",
...
func Test_env_load(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []*config.KeyValue{
				{Key: "SERVICE_NAME", Value: []byte("primus_app"), Format: ""},
...

		{
			...
			args: args{
				envStrings: []string{
					"__SERVICE_NAME=primus_app",
					"__ADDR=192.168.0.1",
					"__AGE=20",
				},
			},
			want: []*config.KeyValue{
				{Key: "_SERVICE_NAME", Value: []byte("primus_app"), Format: ""},
...

		{
			name: "with prefixes",
			fields: fields{
				prefixes: []string{"PRIMUS_", "FOO"},
			},
			args: args{
				envStrings: []string{
					"PRIMUS_SERVICE_NAME=primus_app",
					"PRIMUS_ADDR=192.168.0.1",
					"FOO_AGE=20",
				},
			},
			want: []*config.KeyValue{
