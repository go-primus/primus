...
func (r *reader) Merge(kvs ...*KeyValue) error {
	...
	for _, kv := range kvs {
		...
		if err := r.opts.decoder(kv, next); err != nil {
			slog.Error("Failed to config decode error: ", "err", err, "key", kv.Key, "value", string(kv.Value))
			return err
		}
		if err := r.opts.merge(&merged, convertMap(next)); err != nil {
			slog.Error("Failed to config merge error: ", "err", err, "key", kv.Key, "value", string(kv.Value))
			return err
		}
	}
	r.lock.Lock()
