...
func TestMatch(t *testing.T) {
	tests := []struct {
		...
		{
			name: "/example/primus",
			ctx:  transport.NewServerContext(context.Background(), &Transport{operation: "/example/primus"}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Prefix("/hello/").Regex(`/test/[0-9]+`).
				Path("/example/primus").Build()(next)
...
func TestMatchClient(t *testing.T) {
	tests := []struct {
		...
		{
			name: "/example/primus",
			ctx:  transport.NewClientContext(context.Background(), &Transport{operation: "/example/primus"}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func TestFunc(t *testing.T) {
	tests := []struct {
		...
		{
			name: "/go-primus.dev/primus",
			ctx:  transport.NewServerContext(context.Background(), &Transport{operation: "/go-primus.dev/primus"}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Match(func(_ context.Context, operation string) bool {
				if strings.HasPrefix(operation, "/go-primus.dev") || strings.HasSuffix(operation, "world") {
					return true
				}
				return false
			}).Build()(next)
	...
	tests := []struct {
		...
		},
		{
			...
			ctx: transport.NewServerContext(context.Background(), &Transport{
				...
				headers:   &mockHeader{map[string][]string{"X-Test": {"test2"}, "go-primus": {"primus"}}},
			}),
		},
		{
			name: "/test.Name/1234",
		...
		},
		{
			name: "/go-primus.dev/primus",
			ctx: transport.NewServerContext(context.Background(), &Transport{
				operation: "/go-primus.dev/primus",
				headers:   &mockHeader{map[string][]string{"X-Test": {"test"}}},
			}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Match(func(ctx context.Context, _ string) bool {
				...
				if tr.RequestHeader().Get("go-primus") == "primus" {
					return true
				}
				return false
			}).Build()(next)
